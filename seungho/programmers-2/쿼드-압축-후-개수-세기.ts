...
const compressQuadTree = (arr: number[][]): number[] => {
    const answer: number[] = [0, 0];

    function compress(x: number, y: number, size: number): void {
        if (size === 1) {
            answer[arr[y][x]]++;
            return;
        }
        ...
        const startValue: number = arr[y][x];

        for (let i: number = y; i < y + size; i++) {
            for (let j: number = x; j < x + size; j++) {
