...
먼저 우선순위 큐로 최단거리를 구한 뒤, 다익스트라 알고리즘을 사용해, 마을의 개수를 구한다
*/

type RoadInfo = [number, number, number];
type PriorityQueueItem = {
    value: number;
    priority: number;
};

const delivery = (N: number, road: RoadInfo[], K: number): number => {
    ...
    addToPriorityQueue(pq, 0, 0);

    while (!isQueueEmpty(pq)) {
        const { value: currentVertex, priority: currentDistance } = removeFromPriorityQueue(pq);
        ...
        for (const [adjacent, weight] of graph[currentVertex]) {
            ...
            if (distance < distances[adjacent]) {
                ...
                addToPriorityQueue(pq, adjacent, distance);
            }
        }
    }
...
function addToPriorityQueue(queue: PriorityQueueItem[], value: number, priority: number): void {
    queue.push({ value, priority });
    queue.sort((a, b) => a.priority - b.priority);
}

function removeFromPriorityQueue(queue: PriorityQueueItem[]): PriorityQueueItem {
    return queue.shift()!;
}

function isQueueEmpty(queue: PriorityQueueItem[]): boolean {
