...
const calculateMinimumCrossingTime = (bridge_length: number, weight: number, truck_weights: number[]): number => {
    let time: number = 0;
    let bridge: Truck[] = [];
    let bridge_weight: number = 0;
    ...
    while (truck_weights.length > 0 || bridge.length > 0) {
        ...
        if (bridge.length > 0 && bridge[0].distance === 0) {
            let completedTruck = bridge.shift();
            if (completedTruck) {
                bridge_weight -= completedTruck.weight;
            }
        }

        if (truck_weights.length > 0 && bridge_weight + truck_weights[0] <= weight) {
            let nextTruckWeight = truck_weights.shift();
            if (nextTruckWeight !== undefined) {
                bridge_weight += nextTruckWeight;
                bridge.push({ weight: nextTruckWeight, distance: bridge_length });
            }
        }
    }
